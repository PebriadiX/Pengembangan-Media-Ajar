...
export function EvaluationSection({ questions, resetKey = 0, onNotify, onProgressUpdate }: EvaluationSectionProps) {
  ...
  const [violationAlert, setViolationAlert] = useState<{ message: string; count: number } | null>(null);
  ...
  const [shouldReenterFullscreen, setShouldReenterFullscreen] = useState(false);
  ...
  const handleSubmitRef = useRef<(options?: { reason?: "manual" | "timeout" | "violation" }) => Promise<void>>();
  ...
  const answeredCount = useMemo(() => Object.keys(answers).filter((questionId) => Boolean(answers[questionId])).length, [answers]);
  const missingCount = useMemo(
    () => questions.reduce((count, question) => (answers[question.id] ? count : count + 1), 0),
    [questions, answers]
  );
  const progressPercent = questions.length ? Math.round((answeredCount / questions.length) * 100) : 0;
  const currentQuestion = questions.find((question) => question.id === activeQuestionId) ?? questions[0] ?? null;
  ...
  const getOptionLabel = useCallback((index: number) => OPTION_LABELS[index] ?? String.fromCharCode(65 + index), []);
  ...
  const enterExamFullscreen = useCallback(async () => {
    ...
    if (!(target as HTMLElement & { requestFullscreen?: () => Promise<void> }).requestFullscreen) return;

    try {
      await (target as HTMLElement & { requestFullscreen?: () => Promise<void> }).requestFullscreen();
      setIsFullscreenActive(true);
    } catch {
      setStatusMessage("Browser menolak masuk fullscreen. Izinkan fullscreen untuk melanjutkan ujian.");
    }
  }, []);

  const handleSubmit = useCallback(async (options?: { reason?: "manual" | "timeout" | "violation" }) => {
    if (submitAttemptedRef.current) return;

    let manualStatus = "";
    if (options?.reason === "manual") {
      if (!studentName.trim()) {
        manualStatus += "Nama tidak diisi, dikirim sebagai Peserta anonim. ";
      }
    }
    ...
    setExamEnded(true);
    setExamEndedReason(options?.reason === "timeout" ? "Waktu habis" : options?.reason === "violation" ? "Pelanggaran ujian" : null);
    ...
    const baseMessage = options?.reason === "timeout"
      ? "Waktu habis • "
  ...
  }, [answers, answeredCount, questions.length, score, studentName]);

  // Store latest handleSubmit in ref
  useEffect(() => {
  ...
  const handleViolation = useCallback((reason: string) => {
    ...
    setViolationCount((prev) => {
      ...
      if (nextCount >= MAX_VIOLATIONS) {
        setViolationAlert({
          message: "Pelanggaran maksimum tercapai. Ujian otomatis selesai.",
          count: nextCount,
        });
        if (handleSubmitRef.current) {
          void handleSubmitRef.current({ reason: "violation" });
        }
        setStatusMessage(`Pelanggaran ${nextCount}/${MAX_VIOLATIONS} tercatat. Ujian otomatis selesai.`);
      } else {
        const detailMessage = reason === "shortcut" ? "Aktivitas di luar mode CBT terdeteksi." : "Pindah tab atau layar terdeteksi.";
        setViolationAlert({ message: detailMessage, count: nextCount });
        setStatusMessage(`${detailMessage} Pelanggaran ${nextCount}/${MAX_VIOLATIONS}.`);
      }
      return nextCount;
    });
  }, [isExamActive]);
  ...
  const handleAnswer = (questionId: string, answer: string) => {
    ...
    setStatusMessage("Jawaban tersimpan. Lanjutkan ke soal berikutnya.");
  };

  const handleKirimJawaban = useCallback(() => {
    if (missingCount > 0) {
      setShowIncompletePopup(true);
      setStatusMessage(`Masih ada ${missingCount} jawaban kosong.`);
      return;
    }

    void handleSubmit({ reason: "manual" });
  }, [handleSubmit, missingCount]);

  const startExam = async () => {
    if (started) {
      setStatusMessage("Ujian sedang berjalan. Jangan memulai ujian lagi.");
      return;
    }

    if (submitted || examEnded) {
      setStatusMessage("Ujian sudah selesai. Tidak bisa memulai kembali.");
      return;
    }
    ...
    setStatusMessage("Ujian dimulai. Fokus pada layar dan kerjakan semua soal dengan teliti.");
  ...
  const goToQuestion = (direction: "prev" | "next") => {
    ...
    const nextIndex = direction === "next" ? Math.min(currentIndex + 1, questions.length - 1) : Math.max(currentIndex - 1, 0);
  ...
  useEffect(() => {
    if (!shouldReenterFullscreen || violationAlert) return;

    void enterExamFullscreen();
    setShouldReenterFullscreen(false);
  }, [shouldReenterFullscreen, violationAlert, enterExamFullscreen]);
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      ...
      fullscreenCheckTimeout = setTimeout(() => {
        const isNowActive = document.fullscreenElement === examPanelRef.current;

        if (isFullscreenActiveRef.current && !isNowActive && isExamActiveRef.current) {
    ...
    return () => {
      ...
      if (fullscreenCheckTimeout) clearTimeout(fullscreenCheckTimeout);
      if (document.fullscreenElement === examPanelRef.current) {
        try {
          void document.exitFullscreen();
        } catch {
          // ignore failure when document is no longer active fullscreen
  ...
  useEffect(() => {
    if (!submitted || !examEnded || hasCompletionNotificationFiredRef.current) return;

    hasCompletionNotificationFiredRef.current = true;
    onProgressUpdate?.(8);
    const participantName = studentName.trim() || "Peserta anonim";
    onNotify?.({
  ...
  }, [submitted, examEnded, onProgressUpdate, onNotify, studentName, score]);
  ...
  useEffect(() => {
    ...
    onProgressUpdate?.(2);
  }, [answeredCount, onProgressUpdate, isExamActive]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      const isBlockedShortcut = (event.ctrlKey || event.metaKey) && ["n", "t", "w", "p"].includes(event.key.toLowerCase());
      if (isBlockedShortcut) {
        event.preventDefault();
        handleViolationRef.current("shortcut");
      }

      if (["F11", "Alt", "Tab"].includes(event.key)) {
        event.preventDefault();
        handleViolationRef.current("shortcut");
      }
    ...
    const timer = window.setInterval(() => {
      setTimeRemaining((prev) => {
        if (prev <= 1) {
          ...
          void handleSubmit({ reason: "timeout" });
          return 0;
        }
        ...
        <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
          <div className="max-w-2xl space-y-4">
            ...
            <div className="space-y-2">
              <h2 className="text-3xl font-black tracking-tight text-white">Ujian Evaluasi Siswa</h2>
              <p className="max-w-2xl text-sm leading-6 text-slate-300">
                Ujian berjalan dalam mode fullscreen, memantau fokus layar, dan otomatis menghentikan sesi ketika pelanggaran mencapai batas maksimal.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-[1.5rem] border border-slate-800 bg-slate-900/80 p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.25em] text-slate-400">Jumlah soal</p>
                <p className="mt-3 text-2xl font-black text-white">{questions.length} soal</p>
              </div>
              <div className="rounded-[1.5rem] border border-slate-800 bg-slate-900/80 p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.25em] text-slate-400">Durasi</p>
                <p className="mt-3 text-2xl font-black text-white">{formatTime(EXAM_DURATION_SECONDS)}</p>
              </div>
              <div className="rounded-[1.5rem] border border-slate-800 bg-slate-900/80 p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.25em] text-slate-400">Batas pelanggaran</p>
                <p className="mt-3 text-2xl font-black text-white">{MAX_VIOLATIONS} kali</p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-gradient-to-r from-indigo-500 to-violet-600 px-3 py-1.5 text-xs font-black uppercase tracking-[0.15em] text-white">
                {isExamActive ? "Ujian sedang berjalan" : submitted || examEnded ? "Ujian selesai" : "Siap memulai ujian"}
              </span>
          ...
          <div className="rounded-[2rem] border border-slate-800 bg-slate-900/80 p-5 text-slate-300 shadow-sm">
            <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-400">Aturan CBT</p>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[1.75rem] border border-rose-200 bg-white p-6 shadow-2xl dark:border-rose-900/40 dark:bg-slate-900">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-rose-500">Peringatan CBT</p>
                <h3 className="text-lg font-black text-slate-900 dark:text-white">Pelanggaran terdeteksi</h3>
              </div>
            </div>
            <p className="mt-4 text-sm leading-7 text-slate-600 dark:text-slate-300">{violationAlert.message}</p>
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/75 px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-indigo-500">Hasil ujian</p>
                <h3 className="mt-1 text-xl font-black text-slate-900 dark:text-white">Nilai evaluasi Anda</h3>
              </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/70">
                <p className="text-[10px] font-extrabold uppercase tracking-[0.25em] text-slate-500">Nama</p>
                <p className="mt-1 text-sm font-semibold text-slate-900 dark:text-white">{studentName.trim() || "Peserta anonim"}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/70">
                <p className="text-[10px] font-extrabold uppercase tracking-[0.25em] text-slate-500">Benar</p>
                <p className="mt-1 text-sm font-semibold text-slate-900 dark:text-white">{correctCount}/{questions.length}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/70">
                <p className="text-[10px] font-extrabold uppercase tracking-[0.25em] text-slate-500">Status</p>
                <p className="mt-1 text-sm font-semibold text-slate-900 dark:text-white">{examEndedReason ? "Sesi dihentikan" : "Selesai"}</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/80 px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[2rem] border border-slate-700 bg-slate-900/95 p-6 shadow-2xl shadow-slate-950/40">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-amber-300">Peringatan</p>
                <h3 className="mt-1 text-lg font-black text-white">Masih ada jawaban yang kosong</h3>
              </div>
            </div>
            <p className="mt-4 text-sm leading-7 text-slate-300">
              Lengkapi semua pilihan jawaban sebelum mengirim agar hasil evaluasi dapat diproses dengan benar.
            </p>
            <button
              type="button"
      ...
      {!started && !submitted && !examEnded ? (
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-4 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/70 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Sesi belum dimulai</p>
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Klik tombol mulai ujian untuk memulai sesi CBT. Setelah ujian dimulai atau selesai, sesi tidak bisa dimulai ulang.
              </p>
            </div>
            <button
              type="button"
        ...
        <div ref={examPanelRef} id="cbt-exam-panel" className="grid min-h-[55vh] gap-4 lg:grid-cols-[1.65fr_0.75fr]">
          <div className="flex min-h-0 flex-col rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-5">
            <div className="flex flex-col gap-4 border-b border-slate-100 pb-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-indigo-500">Sesi CBT</p>
                <h3 className="mt-1 text-lg font-extrabold text-slate-900 dark:text-white">Try Out Evaluasi Mandiri</h3>
              </div>
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-600 transition-all duration-300" style={{ width: `${progressPercent}%` }} />
              </div>
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{progressPercent}% selesai</span>
            </div>

            <div className="mt-4 rounded-[1.25rem] border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/70">
              <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">Nama peserta</label>
              <input
                value={studentName}
            ...
            {currentQuestion ? (
              <div className="mt-4 rounded-[1.35rem] border border-slate-200 bg-slate-50/80 p-3 shadow-sm dark:border-slate-800 dark:bg-slate-950/70 sm:p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-indigo-500">Soal {currentIndex + 1}</p>
                    <h4 className="mt-1 text-sm font-extrabold leading-7 text-slate-900 dark:text-white">{currentQuestion.question}</h4>
                  </div>
                </div>

                <div className="mt-4 space-y-2">
                  {currentQuestion.options.map((option, optionIndex) => {
                    const choiceLabel = getOptionLabel(optionIndex);
                    return (
                      <label
                      ...
                      >
                        ...
                        />
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 text-[11px] font-extrabold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{choiceLabel}</span>
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-between">
              <button type="button" onClick={() => goToQuestion("prev")} className="rounded-2xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
                <i className="fa-solid fa-chevron-left mr-2"></i>Sebelumnya
              </button>
              <button type="button" onClick={() => goToQuestion("next")} className="rounded-2xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600">
                Berikutnya<i className="fa-solid fa-chevron-right ml-2"></i>
              </button>
            </div>
          </div>
          ...
          <aside className="w-full space-y-4 rounded-[2rem] border border-slate-200 bg-slate-50 p-3 shadow-sm dark:border-slate-800 dark:bg-slate-950/60 lg:sticky lg:top-4 lg:h-fit lg:max-w-[340px]">
            <div className="rounded-[1.25rem] border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
              <p className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-amber-500">Waktu ujian</p>
              <div className="mt-3 flex items-center justify-between rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-900/50 dark:bg-amber-950/30">
                <span className="text-sm font-semibold text-amber-700 dark:text-amber-300">Sisa waktu</span>
                <span className="text-base font-black text-amber-700 dark:text-amber-300">{formatTime(timeRemaining)}</span>
              </div>
            </div>

            <div className="rounded-[1.25rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
              <p className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-indigo-500">Navigasi soal</p>
              <div className="mt-3 overflow-x-auto">
                <div className="grid min-w-[24rem] grid-cols-[repeat(auto-fit,minmax(2.7rem,1fr))] gap-2">
                  {questions.map((question, index) => {
                  const isActive = question.id === activeQuestionId;
                  const answered = Boolean(answers[question.id]);
                  const selectedOptionIndex = question.options.findIndex((option) => answers[question.id] === option);
                  const selectedLabel = selectedOptionIndex >= 0 ? getOptionLabel(selectedOptionIndex) : null;
                  return (
                    <button
              ...
              >
                {submitted ? "Kirim ulang" : "Kirim jawaban"}
              </button>
            </div>
          </div>
          </aside>
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Ujian selesai</p>
              <h3 className="mt-2 text-xl font-black text-slate-900 dark:text-white">{examEndedReason ? "Sesi dihentikan" : "Jawaban berhasil dikirim"}</h3>
